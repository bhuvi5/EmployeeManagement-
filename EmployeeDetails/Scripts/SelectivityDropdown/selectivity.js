...
function SingleSelectivity(options) {
    ...
    if (!options.positionDropdown) {
        ...
        this.options.positionDropdown = function($el, $selectEl) {
            ...
            $el.css({
                left: position.left + 'px',
                top: (position.top + selectHeight - deltaUp + 14) + 'px'
            }).width(width);
        };
    }
...
var callSuper = Selectivity.inherits(SingleSelectivity, {
    ...
    _resultSelected: function(event) {
        ...
        DropdownChange(event.item, this.$el[0].id);
        this.close();
    }
...
Selectivity.Templates = {
    ...
    dropdown: function(options) {
        ...
        if (options.showSearchInput) {
            ...
            searchInput = (
                ...
                //'</div>'
                 '<div class="selectivity-search-input-container">' +
                    '<input type="text" class="selectivity-search-input" placeholder="Enter search text...">' +
                '</div>'
            );
        }
        return (
        ...
        return (
            '<span class="selectivity-multiple-selected-item' + extraClass + '" ' +
                ...
                (options.removable ? '<a class="selectivity-multiple-selected-item-remove">' +
                                         '<i class="fa fa-remove glyphicon glyphicon-remove"></i>' +
                                     '</a>'
                                   : '') +
                escape(options.text) +
            '</span>'
    ...
    singleSelectInput: (
        '<div class="selectivity-single-select">' +
            '<input type="text" class="selectivity-single-select-input"  style="margin-top:0px">' +
            '<div class="selectivity-single-result-container" style="width:100%"></div>' +
    ...
    singleSelectPlaceholder: function(options) {
        return (
            '<div class="selectivity-placeholder">Click to search</div>'
        );
    },
        ...
        return (
            '<span class="selectivity-single-selected-item" ' +
                ...
                (options.removable ? '<a class="selectivity-single-selected-item-remove">' +
                                         '<i class="fa fa-remove glyphicon glyphicon-remove" style="color:#cccccc" onclick="ClearDropDown();"></i>' +
                                     '</a>'
                                   : '') +
                escape(options.text) +
            '</span>'
    ...
    selectCompliance: function(options) {
        ...
        return ('<select id="' + options.name + '" name="' + options.name + '"' + (options.mode === 'multiple' ? ' multiple' : '') + '></select>');
    },
